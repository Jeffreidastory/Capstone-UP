...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO payment_records (order_id, payment_method, reference_number, payment_proof) VALUES (?, 'gcash', ?, ?)");
    ...
    // Update order status
    $stmt = $conn->prepare("UPDATE orders SET payment_status = 'pending_verification' WHERE id = ?");
